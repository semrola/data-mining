...
def main():
    ...
    if test:
        alphas = [30, 50, 70]
        for a in alphas:
            r = BaggingClassifier(n_estimators=10, max_features=0.5)
            #r = RandomForestClassifier(max_features='sqrt', n_jobs=-1, n_estimators=a)
            CV(Xtrain, Y, r)
        ...
        forest = RandomForestClassifier(max_features='sqrt', n_jobs=-1, n_estimators=50)
        ...
        las = Lasso(alpha=0.03, normalize=True, max_iter=20000)

    learners = [ridge, net, forest, las]
    newY, meanY = stacking(learners, Xtrain, Xtest, Y, Xtrainpca, Xtestpca)

    write_result('./oddaje3.naloga/v5', newY, paramNames, predict)
    write_result('./oddaje3.naloga/v5_mean', meanY, paramNames, predict)
