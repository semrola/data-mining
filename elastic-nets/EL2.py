...
class SelectByPermutationTest:
    ...
    def __call__(self, X, y1):
        ...
        data = [X[:, a] for a, b in enumerate(scores) if b > threshold]
...
def main():
    ...
    l1ratio = 0.1
    normalize = True
    net = ElasticNet(alpha=alpha, l1_ratio=l1ratio, fit_intercept=True, normalize=True, precompute='auto', max_iter=50000)
    #net = Ridge(normalize=True)
    #mean = Orange.regression.RidgeRegressionLearner(alpha = 1)
    #mean = Orange.regression.ElasticNetLearner()

    predictions = []
    for i in range(Y.shape[1]):
        y = Y[:, i]
        fss = SelectByPermutationTest(p=0.5, n_permutations=5)
        Xnew = fss(Xtrain, y)
        print('GHAGAGA', Xnew.shape)
        params = []
        #print('y', y)
        #print('Fitting...')
        net.fit(Xtrain, y)
